...
public class UIManager : MonoBehaviour
{
    ...
    private void AddAssignmentInstructionText()
    {
        if (SceneManager.GetActiveScene().name != "MainMenu" || pages == null)
        {
            return;
        }

        UIPage instructionsPage = pages.Find(page => page != null && page.name == "Instructions");
        if (instructionsPage == null || instructionsPage.transform.Find("Assignment Instructions Text") != null)
        {
            return;
        }

        GameObject textObject = new GameObject("Assignment Instructions Text", typeof(RectTransform), typeof(TextMeshProUGUI));
        textObject.transform.SetParent(instructionsPage.transform, false);

        TextMeshProUGUI instructionText = textObject.GetComponent<TextMeshProUGUI>();
        instructionText.text = "Move: WASD or Arrow Keys\nAim: Mouse\nShoot: Left Mouse Button\nGoal: Defeat enemies and survive.\nCollect yellow Rapid Fire boosts for faster shooting.";
        ...
        RectTransform rectTransform = instructionText.rectTransform;
        rectTransform.anchorMin = new Vector2(0.5f, 0.5f);
        rectTransform.anchorMax = new Vector2(0.5f, 0.5f);
        rectTransform.pivot = new Vector2(0.5f, 0.5f);
        rectTransform.anchoredPosition = new Vector2(0, 15);
        rectTransform.sizeDelta = new Vector2(560, 190);
    }
}
