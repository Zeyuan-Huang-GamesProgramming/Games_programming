...
public static class DeepSpaceStationSceneBuilder
{
    ...
    public static void BuildPlayableScene()
    {
        ...
        Material floor = Mat("M_Floor_DarkSteel", new Color(0.12f, 0.14f, 0.16f));
        Material wall = Mat("M_Wall_BlueGrey", new Color(0.18f, 0.24f, 0.28f));
        Material trim = Mat("M_Trim_Yellow", new Color(0.95f, 0.74f, 0.18f));
        Material terminal = Mat("M_Terminal_Black", new Color(0.02f, 0.025f, 0.03f));
        Material danger = Mat("M_Danger_Red", new Color(1f, 0.12f, 0.06f, 0.35f), true);
        Material pod = Mat("M_EscapePod_White", new Color(0.78f, 0.9f, 0.92f));

        CreateLighting();
        CreateStation(floor, wall, trim, danger);
    ...
    private static void CreateLighting()
    {
        ...
        PointLight("Blue Corridor Light", new Vector3(0f, 3.2f, 0f), new Color(0.25f, 0.75f, 1f), 4f, 18f);
        PointLight("Red Emergency Light", new Vector3(-6f, 2.8f, -7f), new Color(1f, 0.14f, 0.06f), 3f, 12f);
        PointLight("Pod Bay Beacon", new Vector3(0f, 3.2f, -16f), new Color(0.1f, 1f, 0.55f), 3.5f, 12f);
    }
    ...
    private static void CreateStation(Material floor, Material wall, Material trim, Material danger)
    {
        Cube("Main Deck", new Vector3(0f, -0.1f, 0f), new Vector3(18f, 0.2f, 24f), floor);
        Cube("Pod Bay Floor", new Vector3(0f, -0.08f, -16f), new Vector3(8f, 0.2f, 8f), floor);
        Cube("North Wall", new Vector3(0f, 1.6f, 12f), new Vector3(18f, 3.2f, 0.4f), wall);
        Cube("South Wall Left", new Vector3(-6f, 1.6f, -12f), new Vector3(6f, 3.2f, 0.4f), wall);
        Cube("South Wall Right", new Vector3(6f, 1.6f, -12f), new Vector3(6f, 3.2f, 0.4f), wall);
        Cube("West Wall", new Vector3(-9f, 1.6f, 0f), new Vector3(0.4f, 3.2f, 24f), wall);
        Cube("East Wall", new Vector3(9f, 1.6f, 0f), new Vector3(0.4f, 3.2f, 24f), wall);
        Cube("Pod Bay Back Wall", new Vector3(0f, 1.6f, -20f), new Vector3(8f, 3.2f, 0.4f), wall);
        Cube("Pod Bay West Wall", new Vector3(-4f, 1.6f, -16f), new Vector3(0.4f, 3.2f, 8f), wall);
        Cube("Pod Bay East Wall", new Vector3(4f, 1.6f, -16f), new Vector3(0.4f, 3.2f, 8f), wall);
        Cube("Central Reactor Block", new Vector3(0f, 0.8f, -2f), new Vector3(3f, 1.6f, 3f), trim);
        Cube("Broken Cargo A", new Vector3(5.5f, 0.6f, -5f), new Vector3(2f, 1.2f, 2.2f), wall);
        Cube("Broken Cargo B", new Vector3(-5.2f, 0.5f, 4.5f), new Vector3(2.3f, 1f, 1.8f), wall);

        GameObject hazard = Cube("Radiation Leak Trigger", new Vector3(4f, 0.05f, -8f), new Vector3(4f, 0.1f, 4f), danger);
    ...
    private static void CreateHUD()
    {
        ...
        Text oxygen = TextNode(canvasObject.transform, "OxygenText", "O2 100%", new Vector2(24f, -24f), TextAnchor.UpperLeft, 24, font);
        Image oxygenFill = Bar(canvasObject.transform, "OxygenFill", new Vector2(24f, -60f), new Vector2(190f, 14f), new Color(0.05f, 0.95f, 0.8f));
        Text timer = TextNode(canvasObject.transform, "TimerText", "TIME 07:00", new Vector2(24f, -84f), TextAnchor.UpperLeft, 24, font);
        Text repairs = TextNode(canvasObject.transform, "RepairText", "SYSTEMS 0/3", new Vector2(24f, -116f), TextAnchor.UpperLeft, 24, font);
        Text objective = TextNode(canvasObject.transform, "ObjectiveText", "", new Vector2(24f, -152f), TextAnchor.UpperLeft, 20, font);
        Text prompt = TextNode(canvasObject.transform, "PromptText", "", new Vector2(0f, 84f), TextAnchor.LowerCenter, 24, font);
        Text message = TextNode(canvasObject.transform, "MessageText", "", new Vector2(0f, 150f), TextAnchor.LowerCenter, 26, font);
        ...
        Text endTitle = TextNode(panel.transform, "EndTitle", "", new Vector2(0f, 70f), TextAnchor.MiddleCenter, 38, font);
        Text endBody = TextNode(panel.transform, "EndBody", "", new Vector2(0f, -20f), TextAnchor.MiddleCenter, 22, font);
    ...
    private static Text TextNode(Transform parent, string name, string value, Vector2 position, TextAnchor anchor, int size, Font font)
    {
    ...
    {
        GameObject terminal = Cube(name, position, new Vector3(1.4f, 1.8f, 0.7f), material);
        TerminalTask task = terminal.AddComponent<TerminalTask>();
        SetString(task, "systemName", systemName);
        GameObject screen = Cube(name + " Screen", position + new Vector3(0f, 0.25f, -0.38f), new Vector3(0.9f, 0.55f, 0.05f), material);
        screen.transform.SetParent(terminal.transform);
        Light status = PointLight(name + " Status Light", position + new Vector3(0f, 1.25f, -0.3f), Color.red, 2f, 4f);
        status.transform.SetParent(terminal.transform);
    ...
    {
        GameObject frame = Cube(name + " Frame", position, new Vector3(3.2f, 3.2f, 0.35f), trim);
        GameObject panel = Cube(name + " Panel", position, new Vector3(2.2f, 2.8f, 0.45f), wall);
    ...
    {
        GameObject podBody = Cube("EscapePod", position, new Vector3(2.6f, 1.8f, 3.4f), material);
        EscapePod pod = podBody.AddComponent<EscapePod>();
        Light beacon = PointLight("EscapePod Beacon", position + new Vector3(0f, 1.5f, 0f), Color.red, 3.5f, 6f);
