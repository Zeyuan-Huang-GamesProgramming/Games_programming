...
public class GameAudio : MonoBehaviour
{
    ...
    private AudioClip footstepLeftClip;
    private AudioClip footstepRightClip;
    private AudioClip footstepSprintClip;
    private AudioClip footstepCrouchClip;
    ...
    private void Awake()
    {
        ...
        footstepSource = AddSource(false, 0.85f);
    ...
    public void PlayFootstep(bool sprinting, bool crouching, float movementIntensity)
    {
        ...
        if (crouching)
        {
            clip = footstepCrouchClip;
            volume = 0.22f;
            basePitch = 0.86f;
        }
        else if (sprinting)
        {
            clip = footstepSprintClip;
            volume = 0.52f;
            basePitch = 1.1f;
        }
        else
        {
            clip = footstepIndex % 2 == 0 ? footstepLeftClip : footstepRightClip;
            volume = 0.36f;
            basePitch = 1f;
        }
        ...
        footstepSource.pitch = basePitch + UnityEngine.Random.Range(-0.045f, 0.045f);
        footstepSource.PlayOneShot(clip, volume * Mathf.Clamp01(movementIntensity) * masterVolume);
    }
    ...
    private void ApplySourceVolumes()
    {
        ...
        if (footstepSource != null)
        {
            footstepSource.volume = 0.85f * masterVolume;
        }
    }
    ...
    private void BuildProceduralClips()
    {
        ...
        footstepLeftClip = CreateOneShot("Mag Boot Step Left", 0.24f, t =>
            FootstepWave(t, 0.24f, 0.11f, 78f, 390f, 0.31f));
        footstepRightClip = CreateOneShot("Mag Boot Step Right", 0.24f, t =>
            FootstepWave(t, 0.24f, 0.47f, 84f, 430f, 0.29f));
        footstepSprintClip = CreateOneShot("Mag Boot Sprint Step", 0.2f, t =>
            FootstepWave(t, 0.2f, 0.73f, 98f, 520f, 0.38f));
        footstepCrouchClip = CreateOneShot("Soft Mag Boot Step", 0.28f, t =>
            FootstepWave(t, 0.28f, 0.29f, 62f, 300f, 0.18f));
    ...
    private static float FootstepWave(float t, float duration, float seed, float lowFrequency, float metalFrequency, float strength)
    {
        float progress = Mathf.Clamp01(t / duration);
        float heelEnvelope = Mathf.Exp(-progress * 15f);
        float metalEnvelope = Mathf.Exp(-progress * 26f);
        float scrapeEnvelope = Mathf.Clamp01(1f - Mathf.Abs(progress - 0.34f) / 0.26f) * 0.42f;

        float heel = Mathf.Sin(2f * Mathf.PI * lowFrequency * t) * heelEnvelope * strength;
        float metalClick = Mathf.Sin(2f * Mathf.PI * (metalFrequency + progress * 160f) * t) * metalEnvelope * strength * 0.36f;
        float scrape = Noise(seed + t * 105f) * scrapeEnvelope * strength * 0.42f;
        return heel + metalClick + scrape;
    }
}
