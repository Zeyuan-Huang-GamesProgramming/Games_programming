...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Move()
    {
        ...
        UpdateFootsteps(input.magnitude, speed);
    }

    private void UpdateFootsteps(float movementInput, float speed)
    {
        if (!controller.isGrounded || movementInput < 0.08f)
        {
            footstepTimer = 0f;
            return;
        }
        ...
        float speedWeight = Mathf.InverseLerp(crouchSpeed, sprintSpeed, speed);
        footstepTimer += Time.deltaTime * Mathf.Lerp(0.85f, 1.18f, speedWeight);
        ...
        footstepTimer = 0f;
        GameAudio.Instance?.PlayFootstep(IsSprinting, IsCrouching, movementInput);
    }
}
